...
const handleChatLogic = async (message, userId, conversationId = null, userFile) => {
  ...
  try {
    ...
    // --- LOGIKA BARU ---
    // Kasus 1: User mengirim file TANPA pesan teks.
    if (userFile && !message) {
      console.log("INFO: File received without a prompt. Sending acknowledgement.");
      modelResponse = "Baik gambar diterima, silakan ketikan yang mau ditanyakan :)";
      
      if (!conversationId) {
        currentConversationId = uuidv4();
        const title = `Percakapan dengan file: ${userFile.originalname}`;
        await connection.query('INSERT INTO conversations (id, user_id, title) VALUES (?, ?, ?)', [currentConversationId, userId, title]);
      }
      
      await connection.query('INSERT INTO messages (conversation_id, role, content) VALUES (?, ?, ?)', [currentConversationId, 'user', `[File: ${userFile.originalname}]`]);
    ...
    // Kasus 2: User mengirim file DENGAN pesan teks (panggil Gemini)
    if (userFile && message) {
      console.log("INFO: File and prompt detected. Calling Gemini API.");
      const history = conversationId ? (await getChatHistory(conversationId)).map(m => ({ role: m.role, parts: [{ text: m.content }] })) : [];
      const chat = geminiModel.startChat({ history });
      const promptParts = [ message, { inlineData: { data: userFile.buffer.toString("base64"), mimeType: userFile.mimetype } }];
      const result = await chat.sendMessage(promptParts);
      modelResponse = result.response.text();
    } 
    // Kasus 3: User mengirim HANYA teks (panggil FAQ/Groq)
    else if (!userFile && message) {
...
// --- Exports (Tidak ada perubahan di sini) ---
